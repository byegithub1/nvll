...
class Aes256 {
  ...
  static async encrypt(message: string, sharedKey: string): Promise<string> {
    try {
      ...
      const importedKey: CryptoKey = await window.crypto.subtle.importKey(keyType, rawKeyData, importAlgorithm, true, ['encrypt', 'decrypt'])
      ...
      // Encrypt the message.
      const ciphertext: ArrayBuffer = await window.crypto.subtle.encrypt({ name: 'AES-GCM', iv, tagLength: 128 }, importedKey, encoded)
  ...
  static async decrypt(cipher: string, sharedKey: string): Promise<string> {
    try {
      ...
      const importedKey: CryptoKey = await window.crypto.subtle.importKey('raw', rawKeyData, importAlgorithm, true, ['encrypt', 'decrypt'])

      // Decrypt the message.
      const response: ArrayBuffer = await window.crypto.subtle.decrypt({ name: 'AES-GCM', iv, tagLength: 128 }, importedKey, encryptedData)
