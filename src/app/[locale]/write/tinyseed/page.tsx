        ...
        {Array.from({ length: 4 }, (_, j) => {
          ...
          return (
            <Input
              ...
              size="small"
              placeholder="???"
              prefix={<div className={`paragraph !font-semibold ${inputKey.toString().length < 2 && 'mr-1.5'}`}>{inputKey}.</div>}
            ...
            <div>
              ...
              <div className="seed-selector">
                ...
                {proposalButtons.map(
                  (button) =>
                    proposal !== button.key && (
                      <Button className="no-print" key={button.key} type="default" size="small" onClick={() => setProposal(button.key)}>
                        {button.label}
                      </Button>
                    )
                )}
