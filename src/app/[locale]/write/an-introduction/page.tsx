  ...
  return (
    <Container header={header}>
      <main className="paper-an-introduction">
        <Card className="bio-card" hoverable actions={actions} cover={<Image src={photo} alt="User's photo" priority={false} />}>
          <Meta title={paper.about.me.title} avatar={<Avatar className="avatar" src={avatar.src} />} description={paper.about.me.content} />
        </Card>
        <div className="bio-site">
          <h1>{paper.about.site.title}</h1>
          <p>{paper.about.site.content}</p>
          <h1>{paper.about.statement.title}</h1>
          <p>{paper.about.statement.content}</p>
        </div>
      </main>
    </Container>
  )
